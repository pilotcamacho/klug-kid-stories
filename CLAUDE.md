...
**`app/(app)/review/story/actions.ts`** (Server Actions):

`generateProfileTopics(input)` — called once (or when profile changes) to generate 20 story topics via Claude Haiku.
- Input: `{ userProfile: { age?, gender?, interests? } }`
- Prompt asks Claude for 20 diverse topic phrases suited to the profile.
- Output saved to `ProfileTopics` in DynamoDB by the caller; result is reused across sessions.

`generateStory(input)` — called per story group to generate the actual story.
- `GenerateStoryInput` includes `targetWords`, `knownVocab`, `targetLanguage`, `sourceLanguage`, `storyTopic` (one topic drawn at random from `ProfileTopics`), and `userProfile` (age + gender as soft hints only).
- `storyTopic` is the primary theme driver; `userProfile` only influences sentence complexity (age) and character gender.
- System prompt instructs Claude to: always write in target language, use only allowed vocabulary, format blanks as `___ [conjugated-form] (source-translation)`, and be imaginative/fantastical.

**`app/(app)/review/story/page.tsx`:**
- State machine: `loading | generating | active | fallback | complete | empty | error | no_settings`.
- Parallel DynamoDB fetch on mount: `UserSettings`, `UserWordProgress`, `ReviewEvent` (today), `WordMeaning`, `ProfileTopics`.
- After loading, checks if `ProfileTopics` is fresh (profile snapshot matches). If missing or stale, calls `generateProfileTopics` and saves/updates the record in DynamoDB. Non-fatal on failure — stories are still generated without a topic.
- Per group in `startGroup()`: picks a random topic from `topicsRef` and passes it to `generateStory`.
